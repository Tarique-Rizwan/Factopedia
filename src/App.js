...
function Header({ showForm, setShowForm }) {
  ...
  return (
    <header className="header">
      <div className="logo">
        <img src="logo.png" height="68" width="68" alt="Factopedia logo" />
        <h1>{appTitle}</h1>
      </div>
  ...
  return (
    <form className="fact-form" onSubmit={handleSubmit}>
      ...
      <input
        value={source}
        type="text"
        placeholder="Trustworthy sources..."
...
function Fact({ fact, setFacts }) {
  ...
  const isDisputed = fact.votesLiked + fact.votesDisliked < fact.votesFalse;

  async function handleVotes(columnNname) {
    setIsUpdating(true);
    const { data: updatedFact, error } = await supabase
      .from("facts")
      .update({ [columnNname]: fact[columnNname] + 1 })
      .eq("id", fact.id)
      .select();
    setIsUpdating(false);
  ...
  return (
    <li className="fact">
      ...
      <div className="vote-buttons">
        <button onClick={() => handleVotes("votesLiked")} disabled={isUpdating}>
          👍 {fact.votesLiked}
        </button>
        <button
          onClick={() => handleVotes("votesDisliked")}
          disabled={isUpdating}
        >
          👎 {fact.votesDisliked}
        </button>
